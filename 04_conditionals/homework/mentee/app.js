...
// because studentName, maxScore, and passingScore are constants that should not change, while earnedScore and isExtraCredit are variables that may change based on the student's performance or extra credit status.
...
// The order matters because if we check for 80 first, then any score that is 90 or above would also satisfy the condition for 80, and we would incorrectly assign a grade of B instead of A. By checking for the highest threshold first, we ensure that the correct grade is assigned.
...
if (average >= 90) {
    ...
    console.log("Overall Grade: D ⚠️");
} else {
    console.log("Overall Grade: F ❌");
}
